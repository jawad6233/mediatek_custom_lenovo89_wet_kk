	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			dev_dbg(&rmi4_data->i2c_client->dev,
					"%s: SynFinger %d: status = 0x%02x [%4d %4d] [%4d %4d]\n",
					__func__, finger,
					finger_status,
					x, y, wx, wy);
...
//HACK for mtk
	...
	if (button_mask)
		dev_dbg(&rmi4_data->i2c_client->dev,
					"%s: [%d] button_data_buffer = 0x%02x\n",
					__func__, f1a->valid_button_count, button_mask);
#if 0
	for (i = 0; i < MAX_KEY_NUM; i++)
		{
			if (button_mask & (0x01 << i))
			{
				x = tpd_keys_dim_local[i][0];
				y = tpd_keys_dim_local[i][1];

				dev_dbg(&rmi4_data->i2c_client->dev,
					"%s: pressed btn %02x, [%d,%d]\n",
					__func__, i, x, y);

				input_report_abs(rmi4_data->input_dev, ABS_PRESSURE, 20);
                input_report_key(rmi4_data->input_dev, BTN_TOUCH, 1);
//                input_report_key(rmi4_data->input_dev, BTN_TOOL_FINGER, 1);
				input_report_abs(rmi4_data->input_dev, ABS_MT_TOUCH_MAJOR, 1);
...
#endif
	for (button = 0; button < MAX_KEY_NUM; button++) {
		index = button / 8;
		shift = button % 8;
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (!rmi4_data->irq_enabled) {
			...
			//mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, , 1);
...
//	struct synaptics_rmi4_data *rmi4_data;
	...
	thread = kthread_run(touch_event_handler, rmi4_data, TPD_DRIVER_NAME);
	if ( IS_ERR(thread) ) {
...
struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(TPD_DRIVER_NAME, TPD_I2C_ADDR)};
